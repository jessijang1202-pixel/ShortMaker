    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-4">
        <div className="w-full max-w-sm text-center space-y-4">
          <div className="w-16 h-16 bg-[#8BE8AC] rounded-3xl flex items-center justify-center mx-auto shadow-lg">
            <Film className="w-8 h-8 text-[#14351F]" />
          </div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white">회원가입 완료!</h2>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            <strong>{email}</strong> 으로 가입이 완료되었습니다.
          </p>
          <button type="button"
            onClick={() => { setSignupDone(false); setEmailTab('login'); }}
            className="w-full py-3.5 rounded-2xl bg-[#8489F2] hover:bg-[#6D72E0] text-white text-sm font-bold transition-colors shadow-sm">
            로그인하러 가기
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-12">
      ...
      <div className="flex items-center gap-3 mb-8">
        <div className="w-12 h-12 bg-[#241E3C] rounded-2xl flex items-center justify-center shadow-lg">
          <Film className="w-6 h-6 text-[#8BE8AC]" />
        </div>
        <div>
          <h1 className="brand-wordmark text-2xl leading-tight">
      ...
      <div className="w-full max-w-sm space-y-3">
        <p className="text-center text-sm font-semibold text-slate-500 dark:text-slate-400 mb-5">
          간편하게 로그인하고 시작하세요
        </p>

        {error && (
          <div className="flex items-start gap-2 bg-red-50 dark:bg-red-950/30 border-2 border-red-200 dark:border-red-800 rounded-2xl px-3 py-2.5 text-sm text-red-700 dark:text-red-400">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3.5 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl hover:border-[#8489F2]/50 hover:shadow-md transition-all font-bold text-slate-800 dark:text-slate-200 text-sm shadow-sm disabled:opacity-60"
        >
        ...
        <button
          ...
          disabled={!!oauthLoading}
          className="w-full flex items-center gap-3 px-4 py-3.5 bg-[#FEE500] hover:bg-[#F5DB00] rounded-2xl transition-all hover:shadow-md font-bold text-[#3C1E1E] text-sm shadow-sm disabled:opacity-60"
        >
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl border-2 border-[#241E3C]/20 dark:border-white/20 text-sm font-bold text-[#241E3C] dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
        >
        ...
        {/* Email form */}
        {emailOpen && (
          <div className="bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            {/* Sub-tabs */}
            <div className="flex border-b-2 border-slate-200 dark:border-slate-800">
              {(['login', 'signup'] as EmailTab[]).map(t => (
                <button key={t} type="button" onClick={() => switchEmailTab(t)}
                  className={`flex-1 py-3 text-sm font-bold transition-colors ${
                    emailTab === t
                      ? 'text-[#5157D8] dark:text-[#B4B7F8] border-b-2 border-[#8489F2] -mb-0.5 bg-[#ECEDFD] dark:bg-slate-800'
