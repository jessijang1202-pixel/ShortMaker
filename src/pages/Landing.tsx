...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#131122] text-slate-100 overflow-x-hidden font-sans relative">
      
      {/* Background Decorative Blobs */}
      <div className="absolute top-20 left-1/4 w-[400px] h-[400px] bg-[#8489F2]/10 rounded-full blur-[100px] -z-10 pointer-events-none" />
      <div className="absolute top-80 right-1/4 w-[350px] h-[350px] bg-[#8BE8AC]/10 rounded-full blur-[100px] -z-10 pointer-events-none" />
      <div className="absolute bottom-40 left-10 w-[300px] h-[300px] bg-[#F79A4D]/10 rounded-full blur-[90px] -z-10 pointer-events-none" />

      {/* Header */}
      <header className="w-full border-b border-white/5 bg-[#131122]/60 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              id="header_buy_btn"
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white text-slate-950 text-xs font-bold hover:bg-slate-100 transition-colors shadow-lg"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 py-16 sm:py-24 flex flex-col items-center">
        ...
        <div className="flex justify-center mb-6 slide-up">
          <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 text-slate-300 text-xs font-bold px-4 py-2 rounded-full tracking-wide">
        ...
        <section className="text-center max-w-3xl mb-16 slide-up">
          <h1 className="text-5xl sm:text-7xl font-extrabold tracking-tight leading-none mb-6 text-white">
            숏폼 제작소 <span className="bg-gradient-to-r from-[#8BE8AC] via-[#8489F2] to-[#F79A4D] bg-clip-text text-transparent">SnapReel</span>
          </h1>
          <p className="text-lg sm:text-xl font-medium text-slate-300 leading-relaxed max-w-2xl mx-auto">
            매번 머리 싸매며 대본을 고민하고 영상 편집에 시간을 낭비하고 계신가요? 
            <br />
            <span className="text-white font-bold underline decoration-[#8BE8AC] decoration-2 underline-offset-4">
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center items-center w-full max-w-md mx-auto">
            ...
            <Link
              to="/"
              id="hero_demo_btn"
              className="w-full sm:w-auto px-8 py-4 rounded-2xl bg-white/5 hover:bg-white/10 text-white font-bold text-base border border-white/10 transition-all flex items-center justify-center gap-1.5"
            >
              무료 데모 체험
        ...
        <section className="w-full max-w-4xl mb-24 relative slide-up">
          <div className="aspect-[16/9] w-full rounded-[2.5rem] bg-gradient-to-br from-white/10 to-white/5 border border-white/10 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden flex items-center justify-center">
            ...
            <div className="flex w-full h-full gap-6 items-center justify-center px-4 sm:px-12">
              ...
              <div className="hidden md:flex flex-col gap-4 flex-1 text-left">
                ...
                <h3 className="text-2xl font-black text-white leading-tight">자동 타임라인 & 자막 싱크</h3>
              ...
              <div className="w-[220px] sm:w-[240px] h-[360px] sm:h-[400px] shrink-0 rounded-[2.5rem] bg-slate-950 border-[6px] border-slate-800 shadow-2xl relative overflow-hidden flex flex-col">
                {/* Speaker pill */}
                <div className="absolute top-2.5 left-1/2 -translate-x-1/2 w-16 h-4 bg-slate-900 rounded-full z-20 flex items-center justify-center">
        ...
        <section className="w-full max-w-5xl mb-28">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4">크리에이팅의 패러다임을 바꾸는 AI</h2>
            <p className="text-slate-400 text-sm sm:text-base">지루한 노가다는 제시카에게 맡기고 콘텐츠 컨셉에만 집중하세요.</p>
          </div>
          <div className="grid sm:grid-cols-3 gap-6">
            {FEATURES.map((feat, idx) => (
              <div 
                key={idx} 
                className="bg-white/5 border border-white/10 rounded-[2rem] p-8 hover:-translate-y-1 hover:border-white/20 transition-all duration-300 relative group"
              >
        ...
        <section className="w-full max-w-5xl mb-28">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4">단 3단계로 끝나는 제작 흐름</h2>
            <p className="text-slate-400 text-sm sm:text-base">복잡한 영상 편집 툴을 켤 필요가 없습니다.</p>
          </div>
          <div className="grid sm:grid-cols-3 gap-8 relative">
            {STEPS.map((step, idx) => (
              <div key={idx} className="flex flex-col items-start relative pl-6 sm:pl-0">
                {/* Step Connector Line */}
                {idx < 2 && (
                  <div className="hidden sm:block absolute top-7 left-[calc(100%-40px)] w-[80px] h-[1px] bg-gradient-to-r from-slate-700 to-transparent z-0" />
                )}
        ...
        <section className="w-full max-w-4xl bg-gradient-to-r from-[#8489F2]/10 to-[#8BE8AC]/10 border border-white/10 rounded-[2.5rem] p-8 sm:p-12 mb-28 text-center">
          <div className="grid grid-cols-3 gap-6 divide-x divide-white/10">
            <div>
              <p className="text-4xl sm:text-5xl font-black text-white">95%</p>
              <p className="text-xs sm:text-sm font-semibold text-slate-400 mt-2">제작 시간 단축</p>
            </div>
            <div>
              <p className="text-4xl sm:text-5xl font-black text-white">90%</p>
              <p className="text-xs sm:text-sm font-semibold text-slate-400 mt-2">제작 비용 절감</p>
            </div>
            <div>
              <p className="text-4xl sm:text-5xl font-black text-white">24h</p>
              <p className="text-xs sm:text-sm font-semibold text-slate-400 mt-2">쉬지 않는 생성 서버</p>
            </div>
          </div>
        </section>

        {/* Pricing Card */}
        <section className="w-full max-w-md bg-gradient-to-b from-white/10 to-white/5 border border-white/15 rounded-[2.5rem] p-8 sm:p-10 text-center shadow-2xl relative overflow-hidden mb-28">
          <div className="absolute -right-16 -top-16 w-32 h-32 bg-[#8489F2]/20 rounded-full blur-2xl" />
          <div className="absolute -left-16 -bottom-16 w-32 h-32 bg-[#8BE8AC]/20 rounded-full blur-2xl" />

          <p className="text-xs font-extrabold text-[#8BE8AC] tracking-widest uppercase mb-2">LIMITED LAUNCH OFFER</p>
          <h2 className="text-3xl font-extrabold text-white mb-2">스튜디오 라이선스</h2>
          <p className="text-sm text-slate-400 mb-6">추가 결제 없이 무제한으로 기획하고 소장하세요</p>

          <div className="py-6 border-y border-white/10 mb-8">
            <span className="text-sm text-slate-400 line-through mr-2">99,000원</span>
            <span className="text-5xl font-black text-white">
              9,900<span className="text-2xl align-top ml-1">원</span>
            </span>
            <p className="text-xs text-slate-400 mt-2">일회성 구매 · 평생 라이선스 권한 부여</p>
          </div>
            ...
            {FAQS.map((faq, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className="font-bold text-white text-base pr-4 flex items-center gap-2.5">
                  ...
                  {isOpen && (
                    <div className="p-6 pt-0 border-t border-white/5 bg-slate-900/40">
                      <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-line">{faq.a}</p>
                    </div>
                  )}
                </div>
              );
      ...
      {/* Footer */}
      <footer className="w-full border-t border-white/5 bg-[#0e0d18] py-12 text-center text-slate-500 text-xs">
        <div className="max-w-6xl mx-auto px-4 space-y-4">
          <p className="font-black text-sm tracking-tight text-slate-400">
            Snap<span className="text-[#8489F2]">Reel</span>
          </p>
